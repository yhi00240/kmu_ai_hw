...
from practice.hw_service import MNIST
...
class HW(APIView):
    template_name = 'practice/hw/hw.html'
    ...
    @staticmethod
    def draw(request, practice_name):
        template = 'practice/hw/draw.html'
        return render(request, template, {'practice_name': practice_name, 'keys': range(MNIST.NUM_CLASSES)})
    ...
    def draw_result(request, practice_name):
        image_data = eval(request.POST['image_data'])
        mnist = MNIST()
        opt1 = mnist.test_option(image_data, "Single layer", "No", "Sigmoid", "No")
        opt2 = mnist.test_option(image_data, "Multiple layers", "Yes", "ReLU", "Yes")
        reference = mnist.test_reference(image_data)
        print('_single_w-random_sigmoid_connect_all', opt1)
        print('_multi_w-init_relu_dropout', opt2)
        print('reference', reference)
        return HttpResponse(json.dumps({'opt1': opt1, 'opt2': opt2, 'reference': reference}), content_type='application/json')
