...
class MNIST(BasePractice):
    ...
    def run(self, *params):
        ...
        accuracy = accuracy_operation.eval(
            feed_dict={MNIST.X: self.test_data.images, MNIST.Y: self.test_data.labels, MNIST.DROPOUT_RATE: 1},
            session=self.sess
        )
    ...
    def test_option(self, image_data, model_type, weight_initialize, activation_function, dropout):
        save_path, hypothesis, variables = self.get_model(model_type, weight_initialize, activation_function, dropout)
        ...
        return self.sess.run(y, feed_dict={MNIST.X: mnist_data, MNIST.DROPOUT_RATE: 1}).flatten().tolist()

    def test_reference(self, image_data):
        y_ref, variables = high_accuracy.convolutional(MNIST.X, 1.0)
        saver = tf.train.Saver(variables)
        saver.restore(self.sess, "practice/trained_model/convolutional.ckpt")
        mnist_data = image_to_mnist(image_data)
        return self.sess.run(y_ref, feed_dict={MNIST.X: mnist_data, MNIST.DROPOUT_RATE: 1}).flatten().tolist()
